...
var CompeteWalkStepContract = function() {
    ...
    LocalContractStorage.defineProperty(this, "totalWalkerNumber");//池中总的参与人数
...
CompeteWalkStepContract.prototype = {
    ...
    addWalker: function(_nickName, _steps) {
        ...
        if(this.walkerChains.get(this.walkChainNumber-1) != dateStr) {
            ...
            //如果当前是第一条链，还没有完成，则不能记录历史
            //如果当前不是第一条链，而且又创建新链，则记录昨天的历史
            return true;
        } else {
            //添加到当前已有链
    ...
    putRewardForDate:function(_count, _totalAward, _date) {
        ...
        for(var i = 0; i < sortArray.length; i++) {
            ...
            if(i + 1 <= limit) {
                result = Blockchain.transfer(address, perAward1 * 10000000);//一千万wei
                if(!result) {
                    throw new Error("Award transfer failed. Walker Address:" + address + ", NAS(10000000wei):" + perAward1);
                }
            }
            if((i + 1 > limit) && (i + 1 <= 2*limit)) {
                result = Blockchain.transfer(address, perAward2 * 10000000);//一千万wei
                if(!result) {
                    throw new Error("Award transfer failed. Walker Address:" + address + ", NAS(10000000wei):" + perAward2);
                }
            }
            if((i + 1 > 2*limit) && (i + 1 <= 3*limit)) {
                result = Blockchain.transfer(address, perAward3 * 10000000);//一千万wei
                if(!result) {
                    throw new Error("Award transfer failed. Walker Address:" + address + ", NAS(10000000wei):" + perAward3 );
                }
            }
        }
    },
    ...
    _getHistoryWalkerChainFromDate:function(_date) {
        ...
        if(ChainNum == (this.walkChainNumber - 1)) {
            ...
            if(!this.historyDayWalker.get(_date)) {
    ...
    _getSortWalkerChainFromDate:function(_date) {
        if(!this.sortDayWalker.get(_date)){
            var numStr = this.historyDayWalker.get(_date);
            numStr = numStr.substring(0, numStr.length - 1);
            var strArray = numStr.split(",");
            strArray.sort(function(a, b){
                return this.walkerPool.get(parseInt(b)) -this.walkerPool.get(parseInt(a));
            })
